...
def get_train_data(train_images_dir, train_labels_dir, img_h, img_w, N_channels=3, C=2, gt_list = None):
    print('-'*30)
    ...
    total_images -= mean
    total_images = np.transpose(total_images, [0, 1, 2, 3])
...
def get_test_data(test_images_dir, test_labels_dir, img_h, img_w, N_channels=3, C=2):
    print('-'*30)
    ...
    total_images = np.transpose(total_images, [0, 1, 2, 3])  # transpose to shape[N, channels, h, w]
